...
interface Subject {
  ...
  shortName: string;      // e.g. DS, DH, MY, WP
  isLab: boolean;
  prefix: string;         // Code prefix chars
  suffix: string;         // Code suffix chars
}
 ...
 */
const APPS_SCRIPT_URL: string = 'YOUR_APPS_SCRIPT_WEB_APP_URL_HERE';
 ...
 * Example: DS4821LB
 */
function generateCode(subject: Subject): string {
...
function openSheet(overlayId: string, sheetId: string): void {
...
function closeSheet(overlayId: string, sheetId: string): void {
  const overlay = $(overlayId);
  const sheet = $(sheetId);
...
function renderSubjectCards(): void {
  ...
  SUBJECTS.forEach((subject) => {
    ...
    card.addEventListener('keypress', (e: KeyboardEvent) => {
      if (e.key === 'Enter' || e.key === ' ') selectSubject(subject);
    });
...
function selectSubject(subject: Subject): void {
  ...
  ($('sheetSubject') as HTMLElement).textContent = subject.name;
  ($('codeValue') as HTMLElement).textContent = currentCode;

  const today = getTodayISO();
  ($('chipDate') as HTMLElement).textContent = formatDisplayDate(today);
  ($('chipTime') as HTMLElement).textContent = getCurrentTime();
...
async function startAttendance(): Promise<void> {
  if (!activeSubject) return;

  const btn = $('startAttendanceBtn') as HTMLButtonElement;
  btn.disabled = true;
  btn.innerHTML = `
...
function openCheckAttendance(): void {
  if (!activeSubject) return;
  // Pre-fill date
  const picker = $('datePicker') as HTMLInputElement;
  picker.value = getTodayISO();
  ($('checkSubjectLabel') as HTMLElement).textContent = activeSubject.name;
...
async function fetchAttendance(): Promise<void> {
  ...
  try {
    ...
    const json = await res.json() as { records: LectureRecord[] };
...
function renderAttendanceResults(records: LectureRecord[]): void {
  ...
  records.forEach((rec, i) => {
    ...
    row.innerHTML = `
      <div class="result-row-left">
        <div class="rr-subject">${rec.subject}</div>
        <div class="rr-date">${formatDisplayDate(rec.date)}</div>
      </div>
      <div class="result-row-right">
        <div class="rr-code">${rec.code}</div>
        <div class="rr-time">${rec.createdTime}</div>
      </div>
    `;
    list.appendChild(row);
  });

  ($('resultsCount') as HTMLElement).textContent =
...
function setupSwipeToClose(sheetId: string, onClose: () => void): void {
  const sheet = $(sheetId);
  let startY = 0;
